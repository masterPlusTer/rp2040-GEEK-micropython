...
class Cardputer():
    def __init__(self):
        ...
        column_pins = (
            board.KB_COL_0, board.KB_COL_1, board.KB_COL_2, board.KB_COL_3, board.KB_COL_4, board.KB_COL_5,
            board.KB_COL_6)
    ...
    def check_keyboard(self):
        ...
        if events:
            for event in events:
                ...
                if event.pressed:
                    print(f"pressed: {event.key_number} - {self._KEY_MATRIX_LUT[event.key_number]}")
                    ...
                    if new_char in ['SHIFT', 'FN', 'CTRL', 'OPT', 'ALT']:
                        ...
                        if self.shift:
                            new_char = f"{_modifier}{self._KEY_MATRIX_LUT[event.key_number][1]}"
                            self.shift = False
                        elif self.funct:
                            new_char = f"{_modifier}{self._KEY_MATRIX_LUT[event.key_number][2]}"
                            self.funct = False
                    ...
                    if new_char == "SHIFT":
                        ...
                        print(f"function toggle: {self.funct}")
